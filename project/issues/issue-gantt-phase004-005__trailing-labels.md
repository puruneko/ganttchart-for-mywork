...
- Change:
  - `GanttNode` に `trailingLabels?: string[]` を追加。
  - 既存のバー横テキストは無かった（`GanttTaskBar.svelte` のラベルはバー内左寄せの1本のみ）ため、回帰対象は無し。
  - `GanttTimeline.svelte` にて、`trailingLabels` が1件以上ある場合のみ `join(' / ')` で連結して SVG `<text>` を描画。x 座標は `max(バーendのx, milestoneのendのx) + マージン`（milestone がある場合はその右、無ければバー右）。
  - 未指定・空配列時は何も描画しない（回帰ゼロを確認）。
  - フォントサイズはバー内ラベルの0.64倍、色は低コントラスト（`--gantt-trailing-label-color` で上書き可）。
  - 動作確認: `demo/App.svelte` にサンプルタスクを追加し、devサーバー実機で DOM 座標・スタイル・テキスト内容を検証済み（バー自身の日付ラベルが長い場合に視覚的に重なるケースがあるが、これは Issue が指定した「バーendのx+マージン」という位置決めアルゴリズムどおりの結果であり、凝った衝突回避はスコープ外とする Issue の指示に従った）。

- Rationale:
  - 位置決めロジックをシンプルに保つことを Issue 自体が明示的に求めている（「省略記号などの凝った処理はしない」）ため、テキスト幅測定によるオーバーラップ回避は実装しなかった。
