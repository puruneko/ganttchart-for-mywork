...
- Change:
  - `GanttNode` に `milestone?: DateTime | { start: DateTime; end: DateTime }` を追加（issue-phase004-004 確定仕様どおり）。
  - 新規コンポーネント `GanttMilestone.svelte` を追加し、一点は ◆（SVG polygon）、期間は始点・終点の ◆＋間の薄塗り矩形を描画。x 座標計算は既存の `dateToX`（`utils/timeline-calculations.ts`）を必ず経由し、独自計算はしていない。
  - `<title>` によるホバーツールチップ、CSS変数化（`--gantt-milestone-fill` 等）、mousedown ハンドラー非付与（ドラッグ非対象）を実装。
  - `core/data-manager.ts` の `calculateDateRange` を拡張し、`plan` と `milestone`（一点・期間の両方）の日時を範囲計算に算入するようにした（含め漏れ防止）。対応する unit test を `tests/core/data-manager.test.ts` に追加。
  - milestone はバーの有無と独立して描画するため、「due のみ（バー無し）のタスク」でも ◆ が単独表示される。

- Rationale:
  - 座標変換・範囲計算はバー描画と共有関数を使うことで、ズーム・スクロール時のズレ事故を防止（gantt-overview 規約 1・3 に準拠）。
