...
- Change:
  - `src/types.ts`: `GanttExternalDropEvent`・`GanttExternalDragOverEvent` インターフェースを追加
  - `src/types.ts`: `GanttEventHandlers` に `onPanStart` / `onPanEnd` / `onScrollChange` / `onViewportChange` / `onDateRangeChange` / `onExternalDrop` / `onExternalDragOver` を追加
  - `src/types.ts`: `GanttUserEventType` / `GanttUserEventDetailMap` を対応するイベント種別で拡張
  - `src/types.ts`: `panStart` に `originalEvent: MouseEvent` を追加、`panEnd` を `{ endX, endY, originalEvent }` に更新
  - `src/components/GanttChart.svelte`: `handleTimelineDragOver` / `handleTimelineDrop` を実装し timeline wrapper に追加
  - `src/components/GanttChart.svelte`: `handleMouseDown` で `onPanStart` を呼び出し
  - `src/components/GanttChart.svelte`: `handlePanEnd` で `MouseEvent` を受け取り `onPanEnd` を呼び出し
  - `src/components/GanttChart.svelte`: スクロール同期コールバックで `onScrollChange` を呼び出し
  - `src/components/GanttChart.svelte`: ResizeObserver で `onViewportChange` を呼び出し
  - `src/components/GanttChart.svelte`: `extendedDateRange` 変化時に `onDateRangeChange` を呼び出し
  - `src/index.ts`: 新型 `GanttExternalDropEvent` / `GanttExternalDragOverEvent` をエクスポート
  - `tests/core/gantt-event-emitter.test.ts`: 新イベント型のテスト追加、既存 panStart/panEnd テスト更新

- Rationale:
  - 外部プラグイン（markdownEditor-for-mywork）が Gantt チャートへの外部ドロップと座標→日付変換を実現するために必要な API を公開した
