...
- [ ] `ganttchart-for-mywork` に `onBarDragEnd` を追加する（下記「ライブラリ変更 1」参照）
- [ ] `calendar-for-mywork` に `onItemResizeEnd` を追加する（下記「ライブラリ変更 2」参照）
- [ ] `markdownEditor-for-mywork` のプラグイン側を修正する（下記「プラグイン変更」参照）
- [ ] 全テスト実行・パス確認
