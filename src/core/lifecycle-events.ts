...
export class LifecycleEventEmitter extends EventTarget {
  ...
  on(
    ...
    this.addEventListener(`lifecycle:${phase}`, handler as EventListener, options);

    // リスナ削除関数を返す
    return () => this.removeEventListener(`lifecycle:${phase}`, handler as EventListener);
  }
  ...
  once(
    ...
    this.addEventListener(`lifecycle:${phase}`, handler as EventListener, { once: true });
  }
  ...
  onLifecycle(
    ...
    // リスナ削除関数を返す
    return () => this.removeEventListener('lifecycle', handler as EventListener);
  }
